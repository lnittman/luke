...
export const analyzeRepositoryContextTool = createTool({
  ...
  description:
    'Get deep context about a repository including structure, patterns, and quality metrics',
  inputSchema: z.object({
    owner: z.string(),
    repo: z.string(),
...
export const analyzePullRequestTool = createTool({
  ...
  description:
    'Deeply analyze a pull request including code changes, review feedback, and impact',
  inputSchema: z.object({
    owner: z.string(),
    repo: z.string(),
    pullNumber: z.number(),
...
export const detectCodePatternsTool = createTool({
  ...
  description:
    'Detect design patterns, anti-patterns, and architectural decisions in code changes',
  inputSchema: z.object({
    files: z.array(
      z.object({
        filename: z.string(),
...
function performImpactAnalysis(commit: any, files: any[]): any {
  ...
  for (const file of files) {
    ...
    if (file.filename.includes('package.json'))
      patterns.push('dependency-change')
    if (file.patch?.includes('TODO') || file.patch?.includes('FIXME'))
      risks.push('unfinished-work')
...
async function analyzeRepoStructure(
  owner: string,
  repo: string,
  contents: any[]
): Promise<any> {
  ...
  return {
    ...
    hasDocumentation: fileNames.some(
      (name) =>
        name === 'readme.md' || name === 'docs' || name === 'documentation'
    ),
...
function detectPackageManager(files: string[]): string | null {
  ...
  if (files.includes('pipfile') || files.includes('requirements.txt'))
    return 'pip'
...
function analyzePRImpact(pr: any, files: any[], reviews: any[]): any {
  ...
  const hasDocs = files.some(
    (f) => f.filename.includes('README') || f.filename.includes('docs')
  )
  ...
  if (perfKeywords.some((k) => allText.includes(k)))
    performanceImpact = 'improvement'
  if (regressionKeywords.some((k) => allText.includes(k)))
    performanceImpact = 'regression'

  // Check for security keywords
  const securityKeywords = ['security', 'vulnerability', 'CVE', 'patch']
  const riskKeywords = ['bypass', 'expose', 'leak', 'injection']

  if (securityKeywords.some((k) => allText.includes(k)))
    securityImpact = 'improvement'
...
function detectPatterns(
  ...
  if (/any\s*[;,)]/.test(codeBlock) && language === 'TypeScript') {
    ...
    suggestions.push(
      'Consider using unknown or proper type definitions instead of any'
    )
  }
  ...
  if (
    /export\s+(default\s+)?function\s+\w+|export\s+const\s+\w+\s*=/.test(
      codeBlock
    )
  ) {
    patterns.push({
  ...
  for (const pattern of patterns) {
    ...
    if (seen.has(key)) {
      ...
      existing.files = Array.from(
        new Set([...existing.files, ...pattern.files])
      )
