...
async function getGitInfo(
  repoPath: string
): Promise<Partial<DiscoveredRepo> | null> {
  try {
    ...
    return {
      ...
      commitCount: Number.parseInt(commitCount.trim()) || 0,
...
export async function POST() {
  try {
    ...
    const reposWithSettings = repositories.map((repo) => ({
      ...repo,
      enabled: repo.commitCount && repo.commitCount > 5,
    }))
