...
export function useAudioReactivity(options: AudioReactivityOptions = {}) {
  ...
  const [audioLevels, setAudioLevels] = useState<Float32Array>(new Float32Array(dataPoints).fill(0));
  ...
  useEffect(() => {
    ...
    const updateAudioData = () => {
      if (analyserRef.current && dataArrayRef.current) {
        ...
        const normalizedData = dataArrayRef.current.slice(0, dataPoints).map(value => {
          const db = Math.max(value, -90); // Min decibels
          return Math.pow(10, db / 60); // Convert to 0-1 range
        });
        ...
        const bass = normalizedData.slice(0, bassEnd).reduce((a, b) => Math.max(a, b), 0);
        const mid = normalizedData.slice(bassEnd, midEnd).reduce((a, b) => Math.max(a, b), 0);
        const treble = normalizedData.slice(midEnd).reduce((a, b) => Math.max(a, b), 0);
