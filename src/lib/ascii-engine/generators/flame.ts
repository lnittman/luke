  ...
  for (let frame = 0; frame < frameCount; frame++) {
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      if (py >= 0 && py < height && px >= 0 && px < width) {
        if (p.life > 0.7) {
          grid[py][px] = sparkChars[Math.floor(Math.random() * sparkChars.length)];
        } else {
          grid[py][px] = flameChars[Math.floor(p.life * (flameChars.length - 1))];
        }
      }
    }
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const flameIntensity = Math.max(0, 
          1 - (distFromBottom / height) * 1.5 - 
